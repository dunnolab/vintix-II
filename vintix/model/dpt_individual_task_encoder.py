...
class DPTIndividualTaskEncoder(IndividualTaskEncoderNew):
    """Individual task encoder

    Encode sample for DPT according to original paper
    ...
    def __init__(
        ...
        if self.add_bos:
            ...
            self.bos = nn.Parameter(torch.randn(1, self.total_dim))
    ...
    def encode_traj(
        ...
        rews_emb = self.rews_encoders[group_name].forward(
            reward_scale * traj['prev_reward'][1:].squeeze(1))
        ...
        if perm is not None:
            ...
            sars_emb = torch.cat(
                [obs_emb[:obs_emb.shape[0] - 1, :], acs_emb, rews_emb], axis=1
            )

        query_token = torch.cat(
            [obs_emb[-1].reshape((1, obs_emb.shape[1])).to(obs.device),
             torch.zeros(
                 (1, acs_emb.shape[1] + rews_emb.shape[1])
                 ).to(obs.device)],
            dim=1
        )
        ...
        if self.add_bos:
            sars_emb = torch.cat(
                [self.bos, query_token, sars_emb], axis=0
            )
        else:
            sars_emb = torch.cat(
                [query_token, sars_emb], axis=0
            )
