...
class FoundationMapDataset(Dataset):
    ...
    Args:
        ...
        add_demonstrator_actions: Whenever add demonstration action 
            to sample or not
        permute: Whenever generate permutation for sample or not
    """
    ...
    def __init__(self,
                 ...
                 add_demonstrator_actions: bool = False,
                 permute: bool = False):
        ...
        self.dtype = dtype
        self.add_demonstrator_actions = add_demonstrator_actions
    ...
    def __preload(self) -> None:
        ...
        for ds in self.h5_files:
            ...
            for key in ds.keys():
                ...
                if self.add_demonstrator_actions:
                    loaded_ds[key]['demonstrator_actions'] = np.array(
                        gr.get('demonstrator_actions'))
    ...
    def __get_data(self,
        ...
        if self.preload:
            ...
            acs = gr['action']
            if self.add_demonstrator_actions:
                demonstrator_acs = gr['demonstrator_actions']
            else:
                demonstrator_acs = None
            rew = gr['reward']
            stp_num = gr['step_num']
        else:
            ds = self.h5_files[ds_num]
            ...
            if self.add_demonstrator_actions:
                demonstrator_acs = np.array(gr.get('demonstrator_actions'))
            else:
                demonstrator_acs = None
            rew = np.array(gr.get('reward'))
            stp_num = np.array(gr.get('step_num'))
        return obs, acs, rew, stp_num, demonstrator_acs
    ...
    def __prepare_sample(self,
        ...
        for key_num in range(st_key, en_key + 1):
            cur_obs, cur_acs, cur_rew, cur_stp_num, cur_demonstrator_acs = self.__get_data(
                ds_num, key_num)
            obs.append(
                cur_obs
            )
            ...
            demonstrator_acs.append(
                cur_demonstrator_acs
            )
        ...
        demonstrator_acs = np.vstack(demonstrator_acs)
        ...
        perm = None
        demonstrator_action = None

        if self.permute:
            perm = torch.randperm(obs.shape[0] - 2)
        if self.add_demonstrator_actions:
            demonstrator_acs = demonstrator_acs[subsample].astype(self.dtype)
            demonstrator_action = demonstrator_acs[-1]
        ...
        return obs, acs, rew, stp_num, addition_data

    def __getitem__(self,
                    idx: int) -> Dict[str, np.ndarray]:
        obs, acs, rew, stp_num, addition_data = self.__prepare_sample(idx)
        sample = {}
        ...
        if self.permute:
            sample['perm'] = addition_data['perm']
        if self.add_demonstrator_actions:
            sample['demonstrator_action'] = addition_data['demonstrator_action']
...
class FoundationRandomMapDataset(Dataset):
    ...
    def __init__(self,
        ...
        tmp_ds = self.h5_files[0]
        gr = tmp_ds.get(self.keys[0][0])
        groups = list(gr.keys())
        # print("*****DS*****")
        # print(file_paths[0])
        # print("*****GROUPS*****")
        # print(groups)
        if 'expert_action' in groups:
            self.dem_acs_name = 'expert_action'
        elif 'demonstrator_actions' in groups:
            self.dem_acs_name = 'demonstrator_actions'
        else:
            raise Exception('Wrong naming for demonstrator actions')
    ...
    def __get_data(self,
        ...
        if self.preload:
            ...
            stp_num = gr['step_num']
            if self.add_demonstrator_actions:
                demonstrator_acs = gr[self.dem_acs_name]
            else:
                demonstrator_acs = None
        else:
            ds = self.h5_files[ds_num]
            ...
            stp_num = np.array(gr.get('step_num'))
            if self.add_demonstrator_actions:
                demonstrator_acs = np.array(gr.get(self.dem_acs_name))
            else:
                demonstrator_acs = None
        return obs, acs, rew, stp_num, demonstrator_acs
    ...
    def __prepare_sample(self,
        ...
        '''
        Overall scheme of random episode sparsity
        can be non-trivial because of code optimizations
        Here I provide some explanations with additional commentaries
        '''

        shift = left(self.keys[ds_num][st_key])
        cur_ctx_len = self.ep_lens[ds_num][ep_num] - st_t
        len_trans_to_choose = cur_ctx_len
        cur_ep_num = ep_num - 1
        cur_key = st_key

        '''
        Computation of indexes explicitly is unefficient and downgrades performance
        In order to optimize indexes calculation, we store them as continuous cuts (start_i, len_i)
        Where if cut (start_i, len_i) is included, transitions with indexes [start_i, start_i + len_i) 
        are included in sample
        '''
        already_picked = set()
        res_slices = [(0, cur_ctx_len)]

        '''
        First part -- trajectory collection
        At each iteration of cycle we sample step -- how many episodes are we skip to get new episode
        Then we skip episodes and add next episode into the sample
        '''
        while cur_ep_num + 1 < len(self.ep_lens[ds_num]) - 1 and cur_ctx_len < self.context_len:

            step = np.random.geometric(1 / self.ep_sparsity)
            cur_ep_num = min(cur_ep_num + step, len(self.ep_lens[ds_num]) - 2)

            cur_ctx_len += self.ep_lens[ds_num][cur_ep_num +
                                                1] - self.ep_lens[ds_num][cur_ep_num]
            res_slices.append(
                (self.ep_lens[ds_num][cur_ep_num] - st_t,
                 self.ep_lens[ds_num][cur_ep_num + 1] - self.ep_lens[ds_num][cur_ep_num])
            )
            len_trans_to_choose = max(
                len_trans_to_choose, self.ep_lens[ds_num][cur_ep_num + 1] - st_t + 1)
            already_picked.add(cur_ep_num)

        '''
        Second part -- adding episodes in case of insufficient context length
        Here we supplement the sample with random episodes
        '''
        if cur_ctx_len < self.context_len:
            perm = np.random.permutation(
                np.arange(ep_num + 1, len(self.ep_lens[ds_num]) - 1))  # make rand eps included
            for curr_episode in perm:
                if curr_episode in already_picked:
                    continue
                already_picked.add(curr_episode)

                cur_ctx_len += self.ep_lens[ds_num][curr_episode +
                                                    1] - self.ep_lens[ds_num][curr_episode]
                res_slices.append(
                    (self.ep_lens[ds_num][curr_episode] - st_t,
                     self.ep_lens[ds_num][curr_episode + 1] - self.ep_lens[ds_num][curr_episode])
                )
                len_trans_to_choose = max(
                    len_trans_to_choose, self.ep_lens[ds_num][curr_episode + 1] - st_t + 1)

                if cur_ctx_len >= self.context_len:
                    break

            res_slices = sorted(res_slices)

        '''
        Last part -- compression of adjacent cuts
        This part exists in order to optimize sample retrievement from sampled compressed episodes
        '''
        prev_start, prev_len = -1, 0
        filtered_slices = []
        for cur_start, cur_len in res_slices:
            if prev_start + prev_len == cur_start:
                prev_len += cur_len
            else:
                if prev_start >= 0:
                    filtered_slices.append((prev_start, prev_len))
                prev_start, prev_len = cur_start, cur_len

        filtered_slices.append((prev_start, prev_len))

        res_slices = filtered_slices

        while (st_t + res_slices[-1][0] + res_slices[-1][1] - 1 >
               right(self.keys[ds_num][cur_key])):
            cur_key += 1

        for key_num in range(st_key, cur_key + 1):
            cur_obs, cur_acs, cur_rew, cur_stp_num, cur_demonstrator_actions = self.__get_data(
                ds_num, key_num)
            obs.append(
                cur_obs
            )
        ...
        subsample = self.transes[ds_num][st_t:st_t +
                                         len_trans_to_choose] - shift

        subsample = np.hstack([subsample[start: start + len]
                              for start, len in res_slices])
        ...
        return obs, acs, rew, stp_num, np.max(subsample) - np.min(subsample) + 1, addition_data

    def __getitem__(self,
                    idx: int) -> Dict[str, np.ndarray]:
        obs, acs, rew, stp_num, max_trans, addition_data = self.__prepare_sample(
            idx)
        sample = {}
        sample['observation'] = obs[1:]
        sample['prev_action'] = acs[0:-1]
...
class MultiTaskMapDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 ep_sparsity: Union[List[int], int] = 1,
                 last_frac: Optional[float] = None,
                 preload: Union[List[bool], bool] = False,
        ...
        for i, ds_path in enumerate(self.dataset_paths):
            ...
            new_dataset = single_ds_class(
                ...
                add_demonstrator_actions=self.add_demonstrator_actions,
