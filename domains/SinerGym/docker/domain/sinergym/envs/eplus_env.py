...
from domain.sinergym.config import ModelJSON
from domain.sinergym.simulators import EnergyPlus
from domain.sinergym.utils.constants import LOG_ENV_LEVEL
from domain.sinergym.utils.logger import SimpleLogger, TerminalLogger
from domain.sinergym.utils.rewards import *
...
class EplusEnv(gym.Env):
    ...
    def __init__(
        ...
        self.model = ModelJSON(
            ...
            meters=self.meters,
            max_ep_store=self.max_ep_data_store_num,
            experiment_path=self.experiment_path,
            extra_config=self.config_params
        )
        ...
        self.last_action: Optional[Union[List[float], np.ndarray, Tuple[Any], float, int]] = None
    ...
    def reset(
        ...
        # self.model.runperiod = self.model._get_eplus_runperiod()
        ...
        self.last_obs = dict(zip(self.observation_variables, self.observation_space.sample()))
    ...
    def step(self,
        ...
        if not self._action_space.contains(action):
            ...
            raise ValueError(f'Action {action} is not valid for {self._action_space}')

        # check for simulation errors
        if self.energyplus_simulator.failed():
            self.logger.critical(
                f"EnergyPlus failed with exit code {self.energyplus_simulator.sim_results['exit_code']}")
            raise RuntimeError
    ...
    def update_context(self,
        ...
        if len(context_values) != len(self.context):
            self.logger.warning(
                f'Context values must have the same length than context variables specified, and values must be in the same order. The context space is {self.context}, but values {context_values} were spevified.')
    ...
    def _check_eplus_env(self) -> None:
        ...
        if len(self.observation_variables) != self._observation_space.shape[0]:
            self.logger.error(
                f'Observation space ({self._observation_space.shape[0]} variables) has not the same length than specified variable names ({len(self.observation_variables)}).')
            raise ValueError

        # ACTION
        if len(self.action_variables) != self._action_space.shape[0]:
            self.logger.error(
                f'Action space ({self._action_space.shape[0]} variables) has not the same length than specified variable names ({len(self.action_variables)}).')
            raise ValueError
