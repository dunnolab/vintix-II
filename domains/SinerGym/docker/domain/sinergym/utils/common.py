...
import domain.sinergym as sinergym
from domain.sinergym.utils.constants import LOG_COMMON_LEVEL, YEAR
...
def ornstein_uhlenbeck_process(data: pd.DataFrame, variability_config: Dict[str, Tuple[float, float, float]]) -> pd.DataFrame:
...
def parse_actuators_settings(actuators: Dict[str, Dict[str, str]]) -> Dict[str, Tuple[str, str, str]]:
...
def convert_conf_to_env_parameters(conf: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    if len(conf['weather_specification']['weather_files']) != len(
        ...
        logger.error(f'Weather files and id keys must have the same len\n {len(conf["weather_specification"]["weather_files"])} weather files != {len(conf["weather_specification"]["keys"])} keys')
        raise ValueError
...
def process_environment_parameters(env_params: dict) -> dict:  # pragma: no cover
    ...
    if env_params.get('reward'):
        ...
        # if isinstance(env_params['reward'], str):
        #     env_params['reward'] = eval(env_params['reward'])
        # else:
        #     reward_cls = eval(env_params['reward']['class'])
        #     env_params['reward'] = partial(reward_cls, **env_params['reward']['kwargs'])
