...
from domain.sinergym.utils.constants import LOG_CALLBACK_LEVEL
from domain.sinergym.utils.logger import TerminalLogger
from domain.sinergym.utils.wrappers import BaseLoggerWrapper, NormalizeObservation, WandBLogger
...
class LoggerEvalCallback(EventCallback):
    ...
    def __init__(
        ...
        self.save_path = self.train_env.get_wrapper_attr('workspace_path') + '/evaluation'
        ...
        self.evaluation_columns = [col for col in eval_env.get_wrapper_attr('summary_metrics') if col not in excluded_metrics]
        self.evaluation_metrics = pd.DataFrame(columns=self.evaluation_columns)
    ...
    def _on_event(self) -> None:
        ...
        evaluation_summary_df = pd.DataFrame([evaluation_summary]).dropna(axis=1, how="all")

        if not evaluation_summary_df.empty:
            evaluation_summary_df = evaluation_summary_df.reindex(columns=self.evaluation_metrics.columns)
            evaluation_summary_df = evaluation_summary_df.reset_index(drop=True)
            self.evaluation_metrics = self.evaluation_metrics.dropna(axis=1, how="all")
            self.evaluation_metrics = pd.concat([self.evaluation_metrics, evaluation_summary_df], ignore_index=True)
        self.evaluation_metrics.to_csv(self.save_path + '/evaluation_metrics.csv')
        ...
        if self.verbose >= 1:
            self.logger.info(f'Evaluation num_timesteps={self.num_timesteps}, episode_reward={evaluation_summary["mean_reward"]} +/- {evaluation_summary["std_reward"]}')
        ...
        if evaluation_summary['mean_reward'] > self.best_mean_reward:
            ...
            if is_wrapped(self.eval_env, NormalizeObservation):
                ...
                np.savetxt(fname=self.save_path + '/mean.txt', X=self.eval_env.get_wrapper_attr('mean'))
                np.savetxt(fname=self.save_path + '/var.txt', X=self.eval_env.get_wrapper_attr('var'))
            # Save best model found summary in wandb if its active
            if self.wandb_log:
                self.train_env.get_wrapper_attr('_log_data')(data={'best_model': evaluation_summary})
    ...
    def _sync_envs(self):
        # normalization
        if is_wrapped(self.train_env, NormalizeObservation) and is_wrapped(self.eval_env, NormalizeObservation):
            self.eval_env.get_wrapper_attr('deactivate_update')()
            self.eval_env.get_wrapper_attr('set_mean')(self.train_env.get_wrapper_attr('mean'))
            self.eval_env.get_wrapper_attr('set_var')(self.train_env.get_wrapper_attr('var'))
