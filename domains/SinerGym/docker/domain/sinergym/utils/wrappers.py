...
from domain.sinergym.utils.common import is_wrapped, ornstein_uhlenbeck_process
from domain.sinergym.utils.constants import LOG_WRAPPERS_LEVEL, YEAR
from domain.sinergym.utils.logger import LoggerStorage, TerminalLogger
from domain.sinergym.utils.rewards import EnergyCostLinearReward
...
class NormalizeObservation(gym.Wrapper):
    ...
    def __init__(self,
        ...
        self.normalized_space = gym.spaces.Box(
            low=np.array(np.repeat(self.env.get_wrapper_attr('observation_space').low[0], env.get_wrapper_attr('observation_space').shape[0]), dtype=np.float64),
            high=np.array(np.repeat(self.env.get_wrapper_attr('observation_space').high[0], env.get_wrapper_attr('observation_space').shape[0]), dtype=np.float64),
            dtype=np.float64
        )

        # Updated observation_space space to normalized space
        self.observation_space = self.normalized_space
...
class DiscreteIncrementalWrapper(gym.ActionWrapper):
    ...
    def __init__(
        ...
        self.logger.info(self.get_wrapper_attr('mapping'))
        self.logger.info('Wrapper initialized')
...
class LoggerWrapper(BaseLoggerWrapper):
    ...
    def get_episode_summary(self) -> Dict[str, float]:
        ...
        abs_comfort_penalties = [info['abs_comfort_penalty'] for info in self.data_logger.infos[1:]]
        abs_energy_penalties = [info['abs_energy_penalty'] for info in self.data_logger.infos[1:]]
        temperature_violations = [info['total_temperature_violation'] for info in self.data_logger.infos[1:]]
        power_demands = [info['total_power_demand'] for info in self.data_logger.infos[1:]]
        try:
        ...
        data_summary = {
            ...
            'month': self.get_wrapper_attr('runperiod')['end_month'],
            'sum_reward': np.sum(self.data_logger.rewards),
...
try:
    ...
    class WandBLogger(gym.Wrapper):  # pragma: no cover
        ...
        def __init__(self,
            ...
            run_name = run_name if run_name is not None else self.env.get_wrapper_attr('name') + '_' + wandb.util.generate_id()

            # Init WandB session
            # If there is no active run and entity and project has been specified,
            # initialize a new one using the parameters
            if wandb.run is None and (entity is not None and project_name is not None):
        ...
        def step(self, action: Union[int, np.ndarray]
            ...
            if self.global_timestep % self.dump_frequency == 0:
                self.logger.debug(f'Dump Frequency reached in timestep {self.global_timestep}, dumping data in WandB.')
                self.wandb_log()
        ...
        def reset(self,
            ...
            if self.get_wrapper_attr('is_running'):
                ...
                if self.get_wrapper_attr(
                    ...
                    self.logger.warning(f'Episode ignored for log summary in WandB Platform, it has not be completed in at least {self.episode_percentage * 100}%.')
                self.logger.info(
    ...
    class WandBLogger():  # pragma: no cover
        ...
        def __init__(self):
            print('WandB is not installed. Please install it to use WandBLogger.')
...
class VariabilityContextWrapper(gym.Wrapper):
    ...
    def step(self, action: Union[int, np.ndarray]
        ...
        if self.next_step_update == 0:
            ...
            self.logger.info(f'Context updated with values: {self.next_context_values}')
...
class ClipActionWrapper(gym.ActionWrapper):
    ...
    - Box: np.clip to [low, high] and cast to dtype
    - Dict/Tuple: recurse into subspaces
    - Others (Discrete, MultiDiscrete, MultiBinary): pass through
    """
    ...
    def action(self, act):
        return self._clip_to_space(act, self._space)

    def _clip_to_space(self, act, space):
        if isinstance(space, gym.spaces.Box):
            # Convert input to ndarray without copying if possible
            a = np.asarray(act, dtype=space.dtype)
            # Handle infinite bounds by replacing with +/- inf to let np.clip pass values through
            low = np.where(np.isfinite(space.low), space.low, -np.inf)
            high = np.where(np.isfinite(space.high), space.high, np.inf)
            return np.clip(a, low, high).astype(space.dtype, copy=False)

        elif isinstance(space, gym.spaces.Dict):
            # Expect a mapping; clip each field according to its subspace
            if hasattr(act, "items"):
                return {k: self._clip_to_space(act[k], sub) for k, sub in space.spaces.items()}
            # Allow sequence-like dict inputs (e.g., OrderedDict values)
            return {k: self._clip_to_space(v, sub) for (k, v), (k2, sub) in zip(act.items(), space.spaces.items())}

        elif isinstance(space, gym.spaces.Tuple):
            # Expect a sequence; clip per subspace
            return tuple(self._clip_to_space(a_i, s_i) for a_i, s_i in zip(act, space.spaces))

        else:
            # Discrete / MultiDiscrete / MultiBinary or custom: leave unchanged
            return act
