...
from domain.sinergym.utils.common import (
    eppy_element_to_dict,
    get_delta_seconds,
    ornstein_uhlenbeck_process,
)
from domain.sinergym.utils.constants import (
    CWD,
    LOG_MODEL_LEVEL,
    PKG_DATA_PATH,
    WEEKDAY_ENCODING,
...
from domain.sinergym.utils.logger import TerminalLogger
...
class ModelJSON(object):
    ...
    def __init__(
        ...
        # self.experiment_path = tempfile.mkdtemp()
        self.experiment_path = experiment_path
        self.episode_path: Optional[str] = None
        self.max_ep_store = max_ep_store
        self.config = extra_config if extra_config is not None else {}
        ...
        self.logger.info(f'Choosed runperiod {self.runperiod}')
        ...
        def get_day_of_week(year, month, day):
            ...
            day_of_week_number = date_object.weekday()  # Monday is 0, Sunday is 6
            days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
            return days[day_of_week_number]

        runperiod_key = list(self.building['RunPeriod'].keys())[0]
        self.building['RunPeriod'][runperiod_key]['begin_day_of_month'] = self.runperiod['start_day']
        self.building['RunPeriod'][runperiod_key]['begin_month'] = self.runperiod['start_month']
        self.building['RunPeriod'][runperiod_key]['begin_year'] = self.runperiod['start_year']
        self.building['RunPeriod'][runperiod_key]['end_day_of_month'] = self.runperiod['end_day']
        self.building['RunPeriod'][runperiod_key]['end_month'] = self.runperiod['end_month']
        self.building['RunPeriod'][runperiod_key]['end_year'] = self.runperiod['end_year']
        self.building['RunPeriod'][runperiod_key]['day_of_week_for_start_day'] = get_day_of_week(
            self.runperiod['start_year'],
            self.runperiod['start_month'],
            self.runperiod['start_day'],
        )

        # for k in self.runperiod:
        #     self.building['RunPeriod'][runperiod_key][k] = self.runperiod[k]
    ...
    def adapt_building_to_config(self) -> None:
        ...
        if self.config is not None:
            ...
            if self.config.get('timesteps_per_hour'):
                ...
                self.logger.debug(f'Extra config: timesteps_per_hour set up to {self.config["timesteps_per_hour"]}')
            ...
            if self.config.get('runperiod'):
                ...
                self.logger.info(
                    f'Updated timesteps per episode: {self.timestep_per_episode}')
    ...
    def apply_weather_variability(
        ...
        if weather_variability is not None:
            ...
            self.logger.info(f'Weather noise applied in columns: {list(self.weather_variability_config.keys())}')
...
# ---------------------------------------------------------------------------- #
    ...
    def _get_eplus_runperiod(self) -> Dict[str, int]:
        ...
        start_year = int(YEAR if runperiod['begin_year'] is None else runperiod['begin_year'])
        end_year = int(YEAR if runperiod['end_year'] is None else runperiod['end_year'])
        start_weekday = WEEKDAY_ENCODING[runperiod['day_of_week_for_start_day'].lower()]  # TODO: странно что на это не ругается
        n_steps_per_hour = list(self.building['Timestep'].values())[0]['number_of_timesteps_per_hour']

        # month = self.config.get("month", random.randint(1, 12))
        # month = 6

        # if 'weather_time' in self.config:
        #     if self.config['weather_time'] == 'winter':
        #         month = random.choice([12, 1, 2])
        #     elif self.config['weather_time'] == 'spring':
        #         month = random.choice([3, 4, 5])
        #     elif self.config['weather_time'] == 'summer':
        #         month = random.choice([6, 7, 8])
        #     elif self.config['weather_time'] == 'autumn':
        #         month = random.choice([9, 10, 11])
        # else:
        #     month = self.config.get("month", random.randint(1, 12))
        if self.config.get('months'):
            months = {
                'jan': 1,
                'feb': 2,
                'mar': 3,
            ...
            month = random.choice([months[n] for n in self.config['months']])
        else:
    ...
    def set_episode_working_dir(self) -> str:
        ...
        if self.experiment_path is None:
            ...
            episode_path = self._get_working_folder(directory_path=self.experiment_path, base_name='episode-')
            # Create directory
    ...
    def _set_experiment_working_dir(self, env_name: str) -> str:
        ...
        with open(CWD + '/.lock', 'w') as f:
            ...
            try:
                ...
                experiment_path = self._get_working_folder(
                    directory_path=CWD,
                    base_name=f'{env_name}-{self.model_name}-res'
                )
        ...
        self.logger.info('Experiment working directory created.')
        self.logger.info(f'Working directory: {experiment_path}')
    ...
    def _get_working_folder(
        ...
        for folder_name in os.listdir(directory_path):
            ...
            try:
                ...
                folder_num = int(folder_name.split(base_name)[-1])
                if folder_num > experiment_id:
                    experiment_id = folder_num
        ...
        working_dir = os.path.join(directory_path, '%s%d' % (base_name, experiment_id))
        return working_dir
            ...
            for config_key in self.config.keys():
                ...
                if config_key == 'timesteps_per_hour':
                    if self.config[config_key] < 1:
                        self.logger.critical(f'Extra Config: timestep_per_hour must be a positive int value, the value specified is {self.config[config_key]}')
                        raise ValueError
                # Runperiod
                elif config_key == 'runperiod':
                    if not isinstance(self.config[config_key], tuple):
                        self.logger.critical(f'Extra Config: Runperiod specified in extra configuration must be a tuple (type detected {type(self.config[config_key])})')
                        raise TypeError
